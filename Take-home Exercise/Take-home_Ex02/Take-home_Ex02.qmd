...
## 7.0 Hot Spot and Cold Spot Area Analysis (HCSA)
...
```{r}
st_nb$log_revenue_all <- log(st_nb$revenue_all)

# st_nb$year_month <- st_nb$year_month %>% 
  seq_along()
```
...
::: callout-note
Tau = -0.1493 (perfectly decreasing series)

Since the p-value = 0.1279 \> 0.05, we do not reject the null hypothesis that revenue_all is a monotonic decreasing series for 2019 to 2023.
:::
